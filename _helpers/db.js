...
(async function initialize() {
    ...
    // 1️⃣ Create database if it doesn't exist (Only if not using a managed DB like Aiven which usually prevents this)
    try {
        const connection = await mysql.createConnection({ host, port, user, password });
        await connection.query(`CREATE DATABASE IF NOT EXISTS \`${database}\`;`);
        await connection.end();
    } catch (err) {
        console.log('Database creation skipped or failed (common on managed DBs):', err.message);
    }
